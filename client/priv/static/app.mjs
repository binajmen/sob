...
function read_localstorage(key) {
  ...
  return value3 ? new Ok(JSON.parse(value3)) : new Error(void 0);
}
function write_localstorage(key, value3) {
  console.log(value3);
  window.localStorage.setItem(key, JSON.stringify(value3));
}

// build/dev/javascript/app/app.mjs
var Model2 = class extends CustomType {
  constructor(input4, notes) {
    super();
    this.input = input4;
    this.notes = notes;
  }
};
var AddNote = class extends CustomType {
};
var ToggleStatus = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
...
var CacheInput = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var CacheNotes = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
  ...
  return from(
    (dispatch) => {
      if (key === "input") {
        ...
        let _pipe$1 = new CacheInput(_pipe);
        return dispatch(_pipe$1);
      } else if (key === "notes") {
        let _pipe = read_localstorage(key);
        let _pipe$1 = new CacheNotes(_pipe);
        return dispatch(_pipe$1);
      } else {
        return void 0;
      }
...
function init2(_) {
  let model = new Model2("", new_map());
  return [
    model,
    batch(
      toList([read_localstorage2("input"), read_localstorage2("notes")])
    )
  ];
}
...
function update(model, msg) {
  if (msg instanceof AddNote) {
    return [
      new Model2("", insert(model.notes, model.input, false)),
      write_localstorage2("notes", model.notes)
    ];
  } else if (msg instanceof ToggleStatus) {
    let note = msg[0];
    let $ = map_get(model.notes, note);
    if ($.isOk()) {
      let status = $[0];
      return [
        (() => {
          ...
          return new Model2(
            _record.input,
            insert(model.notes, note, !status)
          );
        })(),
        none()
      ];
    } else {
      throw makeError("panic", "app", 55, "update", "Note not found", {});
    }
    ...
    return [
      (() => {
        let _record = model;
        return new Model2(input4, _record.notes);
      })(),
      write_localstorage2("input", input4)
    ];
  } else if (msg instanceof CacheInput && msg[0].isOk()) {
    let input4 = msg[0][0];
    return [
      (() => {
        let _record = model;
    ...
    return [model, none()];
  } else if (msg instanceof CacheNotes && msg[0].isOk()) {
    let notes = msg[0][0];
    debug(notes);
    return [model, none()];
  } else {
...
function view_note(note, status) {
  return li(
    toList([]),
    toList([
      span(toList([]), toList([text2(note)])),
      input3(
        toList([
          type_("checkbox"),
          checked(status),
          on_click(new ToggleStatus(note))
        ])
      )
    ])
  );
}
function view_notes(model) {
  return keyed(
    (_capture) => {
      return ol(toList([]), _capture);
    },
    map(
      map_to_list(model.notes),
      (_use0) => {
        let note = _use0[0];
        let status = _use0[1];
    ...
    toList([
      input3(
        toList([
          value(model.input),
          on_input((var0) => {
            return new UserInput(var0);
          })
        ])
      ),
      button3(
        toList([on_click(new AddNote())]),
        toList([text2("Add")])
      )
    ])
  );
...
function view(model) {
  return div(toList([]), toList([view_notes(model), view_new_note(model)]));
}
function main() {
