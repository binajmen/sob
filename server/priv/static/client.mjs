...
// build/dev/javascript/shared/shared/counter.mjs
...
var Model = class extends CustomType {
  constructor(route, counter, error) {
    super();
    this.route = route;
...
function init2(_) {
  ...
  let route = _block;
  let model = new Model(route, new Counter(0), new None());
  let effect = init(
    (uri) => {
      let _pipe = uri;
...
function view_not_found() {
  return toList([span(toList([]), toList([text3("not found")]))]);
}
function view_session(id) {
  return toList([span(toList([]), toList([text3("session" + id)]))]);
}
function view_about() {
  return toList([span(toList([]), toList([text3("about")]))]);
}
function view_index() {
  return toList([span(toList([]), toList([text3("index")]))]);
}
      ...
      nav(
        ...
        toList([
          ...
          ul(
            ...
            toList([
              view_header_link(new Index2(), model.route, "Index"),
...
function main2() {
  ...
  if (!($ instanceof Ok)) {
    throw makeError(
      ...
      { value: $, start: 475, end: 523, pattern_start: 486, pattern_end: 491 }
    );
  }
  return void 0;
}
